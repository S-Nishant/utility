...
(async () => {
  ...
  user: 'hellocondo_be',
  host: 'localhost',
  database: 'hellocondo_dev',
  password: 'hellocondo_dev@be_updated',
  port: 15432, // default PostgreSQL port
});
  await client.connect();

  const res = await client.query('SELECT * FROM public.niche_pages');
  let urls = res.rows.map(item=>{
    const url = domain + (item.city+"-"+item.state)?.toLowerCase()?.replaceAll(" ","-")+"/n/"+item?.slug;
    item.url = url
    return url;
    // console.log(url+"/n/"+item.slug)
  })
  console.log("Count of urls: " + urls.length)
  await client.end();
...
for (let i = 0; i < urls.length; i += MAX_URLS) {
  ...
  const fileName = `sitemap-niche${i / MAX_URLS + 1}.xml`;
